...
class ViewerPDF {
	...
	private static Logger log = LogManager.getLogger(ViewerPDF.class);
	...
	protected File getFile(String pathName) {
		...
		try {
			...
			throw new RuntimeException("Erro durante a criação do arquivo: [" + pathName + "]. " + "Causado por "
	...
	protected ByteArrayOutputStream getStream() {
		...
		try {
			...
			throw new RuntimeException("Erro durante a criação do stream. " + "Causado por " + e.getLocalizedMessage(),
					e);
	...
	/**
	 * <p>
	 * Verifica se o template que será utilizado virá do resource ou é externo, ou
	 * seja, se o usuário definiu ou não um template.
	 * </p>
	 *
	...
	private void finalizar() throws DocumentException, IOException {
		...
		stamper.setFullCompression();/*
										 * Sets the document's compression to the new 1.5 mode with object streams and
										 * xref streams.
										 */
			...
			if (isNotNull(convenio.getBanco().getImgLogo())) {
				...
				if (BancosSuportados
					...
					if (log.isDebugEnabled()) {
						log.debug(
								"Banco sem imagem da logo informada. " + "Com base no código do banco, uma imagem foi "
	...
	/**
	 * <p>
	 * Coloca as imagens dos campos no pdf de acordo com o nome dos campos do boleto
	 * atribuídos no map e templante.
	 * </p>
	 *
	 * @throws DocumentException
	 * @throws IOException
